...
class logisticRegressionModel:
    ...
    def paint(self, features_test, labels_test, ax, fig):
        features_test = self.normalize(features_test)

        # plot decision boundary
        x1 = features_test[:, 0]
        x2 = features_test[:, 1]

        ax[0].scatter(x1, x2, c=labels_test, cmap='coolwarm')
        ...
        grid = []
        for i in range(min(x1), max(x1), 0.05):
            grid.append([])
            for j in range(min(x2), max(x2), 0.05):
                grid[i].append()
